 ...
 * SAFETY
 * ------
 * Refuses to run against a Redis URL carrying a known production marker, and
 * only ever deletes keys under the `rl:*` auth rate-limit namespaces listed
 * below. It never flushes the database. (In CI the workflow additionally
 * asserts the target is the loopback service container before this runs.)
 */
