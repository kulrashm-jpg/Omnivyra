...
const { runBlogGeneration } = require('../lib/blog/runBlogGeneration');
const { getDefaultBlogTemplates, instantiateBlogTemplate } = require('../lib/blog/defaultBlogTemplates');
const { calculateContentQualityScore } = require('../lib/content/qualityScoringCore');
const { supabase } = require('../backend/db/supabaseClient');

const WORD_TARGETS = [800, 1200, 1600, 2000];
...
async function main() {
  ...
  const minAverage = Number(parseArg('--min-average') || 0);
  const requireAllPass = process.argv.includes('--require-all-pass');
  ...
  const auditRunId = `blog-audit-${Date.now()}`;
  ...
  for (const wordTarget of wordTargets) {
    for (const template of templates) {
      for (const angle of ANGLES) {
        ...
        const row = await runOneCombination(topic, wordTarget, template, angle, companyId, auditRunId);
  ...
  if (companyId === 'audit-company') {
    console.log('Note: could not resolve Omnivyra from the database, so internal-link suggestions may remain expected if no real company posts are available.');
  }
  if (failed.length > 0) {
  ...
  const guardFailures = [];
  if (requireAllPass && failed.length > 0) {
    guardFailures.push(`${failed.length} combination(s) scored below ${threshold}`);
  }
  if (minAverage > 0 && summary.averageScore < minAverage) {
    guardFailures.push(`average score ${summary.averageScore} is below required minimum ${minAverage}`);
  }

  if (guardFailures.length > 0) {
    console.error(`\n[audit-blog-combinations] Guard failed: ${guardFailures.join('; ')}`);
    process.exit(1);
  }
}
