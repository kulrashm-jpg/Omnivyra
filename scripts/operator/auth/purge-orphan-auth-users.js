 ...
 *
 * Usage:
 *   node scripts/operator/auth/purge-orphan-auth-users.js           # dry-run
 *   node scripts/operator/auth/purge-orphan-auth-users.js --execute # actually delete
 */
...
const EXECUTE = process.argv.includes('--execute')
  ...
  || process.argv.includes('--i-understand-this-mutates-data');

async function main() {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/auth/purge-orphan-auth-users.js',
    mutationTarget: 'auth',
    intendedAction: 'scan for orphan Supabase Auth accounts and delete them when execute/apply intent is present',
    example: 'node scripts/operator/auth/purge-orphan-auth-users.js --target-env=local --execute',
  });
