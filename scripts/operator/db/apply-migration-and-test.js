...
async function applyMigrationAndTest() {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/db/apply-migration-and-test.js',
    mutationTarget: 'db/schema',
    intendedAction: 'inspect/apply integration migration guidance and run module tests against the configured database',
    example: 'node scripts/operator/db/apply-migration-and-test.js --target-env=local --apply',
  });
