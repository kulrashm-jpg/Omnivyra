...
/**
 * scripts/operator/db/backfillCanonicalDomains.ts
 *
 ...
 *
 * Usage:
 *   npx ts-node scripts/operator/db/backfillCanonicalDomains.ts [--dry-run] [--report=path.json]
 *
...
async function main() {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/db/backfillCanonicalDomains.ts',
    mutationTarget: 'db',
    intendedAction: 'resolve and update system-created company domain canonicalization records',
    example: 'npx tsx scripts/operator/db/backfillCanonicalDomains.ts --target-env=local --apply',
  });
