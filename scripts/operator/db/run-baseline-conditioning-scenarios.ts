 ...
 *
 * Run: npx ts-node scripts/operator/db/run-baseline-conditioning-scenarios.ts
 * Or:  npx tsx scripts/operator/db/run-baseline-conditioning-scenarios.ts
 */
...
async function main() {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/db/run-baseline-conditioning-scenarios.ts',
    mutationTarget: 'db',
    intendedAction: 'create temporary campaign scenario rows and remove them after baseline conditioning checks',
    example: 'npx tsx scripts/operator/db/run-baseline-conditioning-scenarios.ts --target-env=local --apply',
  });
