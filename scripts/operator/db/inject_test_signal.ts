...
async function main() {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/db/inject_test_signal.ts',
    mutationTarget: 'db',
    intendedAction: 'insert a synthetic lead signal into the configured database',
    example: 'npx tsx scripts/operator/db/inject_test_signal.ts --target-env=local --apply',
  });
