...
async function main() {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/db/backfill_lead_signals.ts',
    mutationTarget: 'db',
    intendedAction: 'backfill canonical lead signals from listening and engagement signal tables',
    example: 'npx tsx scripts/operator/db/backfill_lead_signals.ts --target-env=local --apply',
  });
