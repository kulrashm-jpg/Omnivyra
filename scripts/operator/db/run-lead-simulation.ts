 ...
 *   node -r ts-node/register scripts/operator/db/run-lead-simulation.ts
 */
...
async function main() {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/db/run-lead-simulation.ts',
    mutationTarget: 'db',
    intendedAction: 'insert a synthetic lead job and process it against the configured database',
    example: 'npx tsx scripts/operator/db/run-lead-simulation.ts --target-env=local --apply',
  });
  ...
  if (roleRow?.company_id) {
    ...
    const { data: jobRow } = await ownedDbTable('lead_jobs_v1')
      .select('company_id')
      .limit(1)
      .maybeSingle();
  ...
  // 2) Create job
  const { data: job, error: insertErr } = await ownedDbTable('lead_jobs_v1')
    .insert({
  ...
  for (let i = 0; i < 15; i++) {
    await sleep(2000);
    const { data: updated } = await ownedDbTable('lead_jobs_v1')
    ...
    if (updated.status === 'COMPLETED' || updated.status === 'COMPLETED_WITH_WARNINGS' || updated.status === 'FAILED') {
      ...
      const { data: leads } = await ownedDbTable('lead_signals_v1')
