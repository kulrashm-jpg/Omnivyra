 ...
 *
 * Usage:
 *   npx ts-node scripts/operator/db/backfill-unified-transactions.ts [--since=ISO] [--until=ISO] [--dry-run]
 */
...
async function run(): Promise<void> {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/db/backfill-unified-transactions.ts',
    mutationTarget: 'db/billing-ledger',
    intendedAction: 'backfill unified_transactions rows from legacy usage and credit ledger tables',
    example: 'npx tsx scripts/operator/db/backfill-unified-transactions.ts --target-env=local --apply',
  });
