...
async function main() {
  const safety = enforceOperatorSafety({
    scriptName: 'scripts/operator/db/backfill_contacts.ts',
    mutationTarget: 'db',
    intendedAction: 'upsert contact records and link existing lead signal rows to contacts',
    example: 'npx tsx scripts/operator/db/backfill_contacts.ts --target-env=local --apply',
  });
