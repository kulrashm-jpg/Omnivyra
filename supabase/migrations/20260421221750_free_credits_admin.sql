...
-- Version: 20260421221750  Name: free_credits_admin
-- Idempotency: GUARDED (CREATE TABLE IF NOT EXISTS, ADD COLUMN IF NOT EXISTS, CREATE OR REPLACE).
-- Note: includes data UPDATE (user_company_roles role rename). Re-runs are no-ops.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname='public' AND tablename='manual_credit_grants' AND policyname='service_role_all'
  ) THEN
