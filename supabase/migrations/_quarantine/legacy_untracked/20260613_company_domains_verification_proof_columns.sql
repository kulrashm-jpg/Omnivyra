...
-- (in saveDomainRecord). The user proves ownership by publishing the token
-- via either:
--
--   A) DNS TXT record:  omnivira-verification=TOKEN  (on the domain apex)
...
-- gate (they use verification_status='admin_override' or are pre-trusted).
--
-- Soft enforcement only — usage is not blocked for unverified rows yet.
-- This migration is additive; no existing data is mutated.
--
