...
--
-- SAFE MIGRATION â€” no data deleted, backward compatible until code switch.
--
-- Changes:
--   1. users: add supabase_uid, active_company_id, onboarding_state,
--             has_password, signup_source
--   2. users: copy company_id â†’ active_company_id (keep company_id for compat)
...
-- 1. USERS TABLE â€” new columns
-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

-- 1a. supabase_uid â€” links to auth.users identity (code already uses this column
...
-- 1b. active_company_id â€” replaces company_id for multi-tenant support
...
-- 1c. Copy existing company_id â†’ active_company_id (only where not already set)
UPDATE users
  SET active_company_id = company_id
  WHERE company_id IS NOT NULL
    AND active_company_id IS NULL;

-- 1d. onboarding_state â€” tracks user lifecycle stage
ALTER TABLE users
  ADD COLUMN IF NOT EXISTS onboarding_state TEXT NOT NULL DEFAULT 'active';

-- 1e. has_password â€” whether user has set a password (vs magic-link only)
ALTER TABLE users
  ADD COLUMN IF NOT EXISTS has_password BOOLEAN NOT NULL DEFAULT false;

-- 1f. signup_source â€” how the user originally registered
ALTER TABLE users
...
-- company_id will NOT be kept in sync â€” it is frozen at its current value.
-- A future migration will drop company_id after all code references are removed.

COMMENT ON COLUMN users.company_id IS
  'DEPRECATED â€” use active_company_id. This column is frozen and no longer updated.';

-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
-- 2. COMPANIES TABLE â€” deprecate admin_email_domain (keep for compat)
-- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
COMMENT ON COLUMN companies.admin_email_domain IS
  'DEPRECATED â€” migrated to company_domains table. Do not write new code against this column.';
...
-- 4a. Migrate from companies.admin_email_domain â†’ company_domains
...
SELECT DISTINCT ON (u.company_id)
  ...
  true,
  false,                           -- unverified â€” inferred from user email
  NOW()
FROM users u
WHERE u.company_id IS NOT NULL
  ...
  AND SPLIT_PART(u.email, '@', 2) NOT IN (
    -- Exclude personal email providers â€” these are not company domains
...
-- 7b. Handle duplicate emails â€” keep the latest active row, soft-delete the rest
...
-- 7c. Fix orphan users â€” users with a company_id that doesn't exist in companies
UPDATE users
  SET company_id = NULL,
...
-- 7d. Fix orphan user_company_roles â€” roles pointing to non-existent companies
DELETE FROM user_company_roles
  WHERE NOT EXISTS (
    SELECT 1 FROM companies c WHERE c.id = user_company_roles.company_id
  );

-- 7e. Fix orphan user_company_roles â€” roles pointing to non-existent users
DELETE FROM user_company_roles
  WHERE NOT EXISTS (
...
-- 8a. users.email â€” ensure UNIQUE (should exist from base schema, re-assert)
...
-- 8b. company_domains.domain UNIQUE â€” already created via table constraint above
