...
-- Version: 20260501115028  Name: domain_reminders_pg_cron_schedule
-- Idempotency: GUARDED (CREATE EXTENSION IF NOT EXISTS, CREATE OR REPLACE FUNCTION, unschedule-then-schedule pattern).
-- Requires extensions: pg_cron, pg_net, supabase_vault.
...
-- Wrapper the cron job calls. Reads URL + bearer from vault.decrypted_secrets
-- so the cron.job table never holds the secret in plaintext.
...
-- One-shot configuration helper. Call once after migration to register the
-- URL + bearer secret. Idempotent — re-running rotates the values.
...
-- (Re-)schedule the cron job. Idempotent: unschedule existing entry first.
DO $$
BEGIN
