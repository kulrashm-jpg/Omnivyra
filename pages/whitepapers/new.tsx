    ...
    try {
      ...
      if (output) {
        if (typeof window !== 'undefined') {
          ...
          const resolvedBlocks = resolveGeneratedPrefillBlocks(output, DEFAULT_TEMPLATE);
          console.info('[whitepapers/new] prefill-received', {
            titleLength: typeof outputRecord.title === 'string' ? outputRecord.title.length : 0,
            excerptLength: typeof outputRecord.excerpt === 'string' ? outputRecord.excerpt.length : 0,
            incomingBlockCount: Array.isArray(outputRecord.content_blocks) ? outputRecord.content_blocks.length : 0,
            incomingHtmlLength: typeof outputRecord.content_html === 'string' ? outputRecord.content_html.length : 0,
            incomingMarkdownLength: typeof outputRecord.content_markdown === 'string' ? outputRecord.content_markdown.length : 0,
            resolvedBlockCount: resolvedBlocks.length,
          });
        }
        setPrefillInitial({
          title: output.title || '',
          ...
          content_blocks: realizeGeneratedBlocks(output, DEFAULT_TEMPLATE, { contentType: 'whitepaper', documentTitle: output.title }),
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 rounded-lg border border-gray-200 bg-white p-6 shadow">
              ...
              <EditorShareActions
                ...
                exportFileName={`${(liveState?.title || 'whitepaper').toLowerCase().replace(/\s+/g, '-')}.md`}
