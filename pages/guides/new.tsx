...
export default function GuideNewPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/company/blogs?company_id=${selectedCompanyId}&content_type=guide`, { credentials: 'include' })
  ...
  const handleSubmit = async (state: BlogFormState) => {
    if (!selectedCompanyId) {
      setError('Company context required to save guide.');
      return;
    }
    ...
    try {
      ...
      setPrefillNotice('Guide saved as draft. Use the options below to share or export.');
    } catch (e) {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `guide-${(liveState.title || 'pillar').toLowerCase().replace(/\s+/g, '-')}.md`;
    a.click();
  ...
  const handleMarkUsed = async (platform?: string) => {
    if (!savedId || !selectedCompanyId) {
      setError('Save the guide first before marking as used.');
      return;
    }
    setMarkingUsed(true);
    try {
      ...
      setPrefillNotice(`Guide marked as used${platform ? ` on ${platform}` : ''}. Great work!`);
  ...
  return (
    <>
      <Head>
        <title>New Guide | Pillar Content</title>
      </Head>
      <div className="min-h-screen bg-gray-50">
        <div className="border-b border-gray-200 bg-white px-4 py-3 sm:px-6">
          <div className="mx-auto flex max-w-[1200px] items-center justify-between">
            ...
            <Link href="/guides/generate" className="text-sm font-medium text-gray-600 hover:text-gray-900">
              &larr; Back to Guide Intelligence
            </Link>
          </div>
        </div>

        <div className="mx-auto max-w-[1200px] p-6">
          <h1 className="mb-6 text-2xl font-bold text-gray-900">New Guide</h1>
          {prefillNotice && (
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 rounded-lg border border-gray-200 bg-white p-6 shadow">
              <BlogEditorForm
                ...
                onCancel={() => router.push('/command-center/content')}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg bg-violet-600 text-white hover:opacity-90 transition-colors disabled:opacity-50"
                    >
