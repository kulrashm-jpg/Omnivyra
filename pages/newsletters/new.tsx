...
export default function NewsletterNewPage() {
  ...
  const autoImproveArea = async (area: ImproveArea) => {
    ...
    try {
      const resp = await fetch('/api/content/improve-draft', {
        ...
        body: JSON.stringify({
          ...
          draft: {
            ...
            format_type: liveState.format_type,
          },
        }),
      });
      ...
      setPrefillNotice(
        `AI improved ${area}. Review the updated newsletter and quality panel for the refreshed score.`,
      );
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (output) {
        ...
        if (parsed.source === 'newsletter_template_flow') {
          setPrefillNotice(
            parsed.template_name
              ? `Draft prefilled from ${parsed.template_name}. Review, strengthen, and save when ready.`
              : 'Draft prefilled from your newsletter flow. Review and save when ready.',
          );
        }
      }
  ...
  const handleSubmit = async (state: BlogFormState) => {
    if (!selectedCompanyId) {
      setError('Company context required to save newsletter.');
      return;
    }
    ...
    try {
      ...
      setPrefillNotice('Newsletter saved as draft. Use the options below to share or export.');
    } catch (e) {
  ...
  const handleCreateCampaign = () => {
    ...
    launchCampaignFromContent({
      ...
      tags: liveState.tags,
      targetWordCount,
      formatType: liveState.format_type || formatType || undefined,
      sourceId: savedId,
  ...
  return (
    <>
      <Head>
        <title>New Newsletter | Content Intelligence</title>
      </Head>
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="mx-auto max-w-[1200px] p-6">
          <h1 className="mb-6 text-2xl font-bold text-gray-900">New Newsletter</h1>
          {prefillNotice && (
              ...
              {liveState && (
                <ContentQualityPanel
                  ...
                  formState={{
                    ...
                    target_word_count:    targetWordCount,
                    content_type:         'newsletter',
                    format_type:          liveState.format_type || formatType || undefined,
                  }}
