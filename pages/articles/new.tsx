...
export default function ArticleNewPage() {
  ...
  const autoImproveArea = async (area: ImproveArea) => {
    ...
    try {
      ...
      setPrefillNotice(
        delta > 0
          ? `AI improved ${area}. Score +${delta} (now ${after}/100). Review content when ready.`
  ...
  useEffect(() => {
    ...
    fetch(`/api/company/blogs?company_id=${selectedCompanyId}&content_type=article`, { credentials: 'include' })
  ...
  const handleSubmit = async (state: BlogFormState) => {
    if (!selectedCompanyId) {
      setError('Company context required to save article.');
      return;
    }
    ...
    try {
      ...
      setPrefillNotice('Article saved as draft. Use the options below to share or export.');
    } catch (e) {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `${(liveState.title || 'article').toLowerCase().replace(/\s+/g, '-')}.md`;
    a.click();
  ...
  const handleMarkUsed = async (platform?: string) => {
    if (!savedId || !selectedCompanyId) {
      setError('Save the article first before marking as used.');
      return;
    }
    setMarkingUsed(true);
    try {
      ...
      setPrefillNotice(`Article marked as used${platform ? ` on ${platform}` : ''}. Great work!`);
  ...
  return (
    <>
      <Head>
        <title>New Article | Content Intelligence</title>
      </Head>
      <div className="min-h-screen bg-gray-50">
        <div className="border-b border-gray-200 bg-white px-4 py-3 sm:px-6">
          <div className="mx-auto flex max-w-[1200px] items-center justify-between">
            ...
            <Link href="/articles" className="text-sm font-medium text-gray-600 hover:text-gray-900">
        ...
        <div className="mx-auto max-w-[1200px] p-6">
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 rounded-lg border border-gray-200 bg-white p-6 shadow">
              ...
              {/* ── Use / Share Actions (replaces Publish) ──────────────── */}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg bg-[#0B5ED7] text-white hover:opacity-90 transition-colors disabled:opacity-50"
                    >
                    ...
                    {showUseMenu && (
                      <div className="absolute top-full left-0 mt-1 w-44 bg-white border border-gray-200 rounded-lg shadow-lg z-20 py-1">
                        ...
                        <button onClick={() => handleMarkUsed('twitter')} className="block w-full text-left px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50">Twitter / X</button>
                        <button onClick={() => handleMarkUsed('newsletter')} className="block w-full text-left px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50">Newsletter</button>
                        <button onClick={() => handleMarkUsed('email')} className="block w-full text-left px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50">Email outreach</button>
                      </div>
                    )}
                  </div>
                </div>
