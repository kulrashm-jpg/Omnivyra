...
import {
  runThreadGeneration,
  type ThreadGenerationRequest,
} from '../../../lib/thread/runThreadGeneration';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    } satisfies ThreadGenerationRequest);

    return res.status(200).json(result);
  } catch (error) {
    console.error('[threads/generate] runThreadGeneration error:', error);
...
export default __createApiRoute(handler, { route: '/api/threads/generate' });
