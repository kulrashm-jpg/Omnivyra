...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // The computation lives in a service (SEC-E1, STEP 3AH-91) so
  // propose-frequency-rebalance can reuse it in-process instead of calling
  // this route over HTTP.
  const advice = await computePlatformAllocationAdvice(id);
...
export default __createApiRoute(handler, { route: '/api/campaigns/:id/platform-allocation-advice' });
