...
import { buildExecutiveAlerts } from '../../../backend/services/networkIntelligence/executiveAlertsService';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await buildExecutiveAlerts({
    ...
    return res.status(500).json({ error: error?.message || 'FAILED_TO_LOAD_EXECUTIVE_ALERTS' });
  }
}

// W0-1 (Gate A): canonical route pipeline — pass-through observability + request context.
export default __createApiRoute(handler, { route: '/api/community-ai/executive-alerts' });
