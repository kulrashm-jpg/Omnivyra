...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (error) {
    ...
    logger.error('oauth.callback.failure', { platform: 'linkedin', flow: 'community-ai', reason: 'oauth_provider_error', provider_error: String(error), provider_error_description: error_description ? String(error_description) : null });
    return res.redirect(
  ...
  if (!code || typeof code !== 'string') {
    logger.error('oauth.callback.failure', { platform: 'linkedin', flow: 'community-ai', reason: 'no_code' });
    return res.redirect(
  ...
  if (!state || typeof state !== 'string') {
    logger.error('oauth.callback.failure', { platform: 'linkedin', flow: 'community-ai', reason: 'no_state' });
    return res.redirect(
  ...
  try {
    ...
    logger.info('oauth.callback.success', { platform: 'linkedin', flow: 'community-ai', user_id: access!.userId, organization_id: organizationId, linkedin_sub: linkedinSub });

    return res.redirect(
      `${redirectTo}?connected=linkedin&status=success`
    );
  } catch (err: any) {
    logger.error('oauth.callback.failure', { platform: 'linkedin', flow: 'community-ai', reason: 'exception', error_message: err?.message ?? 'unknown' });
    return res.redirect(
      `/community-ai/connectors?error=${encodeURIComponent('Connection failed. Please try again.')}`
    );
  }
