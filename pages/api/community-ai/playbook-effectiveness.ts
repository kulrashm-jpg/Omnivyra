...
import {
  fetchPlaybookEffectiveness,
  type PlaybookEffectivenessFilters,
} from '../../../backend/services/networkIntelligence/playbookEffectivenessService';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const filters: PlaybookEffectivenessFilters = {
  ...
  try {
    const result = await fetchPlaybookEffectiveness(filters);
    ...
    return res.status(500).json({ error: error?.message || 'FAILED_TO_LOAD_PLAYBOOK_EFFECTIVENESS' });
  }
}

// W0-1 (Gate A): canonical route pipeline — pass-through observability + request context.
export default __createApiRoute(handler, { route: '/api/community-ai/playbook-effectiveness' });
