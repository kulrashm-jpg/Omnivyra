...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const narrative = await buildExecutiveNarrative({
    ...
    return res
      .status(500)
      .json({ error: error?.message || 'FAILED_TO_LOAD_EXECUTIVE_NARRATIVE' });
  }
}

// W0-1 (Gate A): canonical route pipeline — pass-through observability + request context.
export default __createApiRoute(handler, { route: '/api/community-ai/executive-narrative' });
