...
import { buildPlaybookLearning } from '../../../backend/services/networkIntelligence/playbookLearningService';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await buildPlaybookLearning({
    ...
    return res.status(500).json({ error: error?.message || 'FAILED_TO_LOAD_PLAYBOOK_LEARNING' });
  }
}

// W0-1 (Gate A): canonical route pipeline — pass-through observability + request context.
export default __createApiRoute(handler, { route: '/api/community-ai/playbook-learning' });
