...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const platform = 'x';
    const { valid, companyId, userId: stateUserId, returnTo, flow: stateFlow, tenantId: stateTenantId } = decodeOAuthState(state as string);
    if (valid === false) {
      logOAuthEvent({
        event: 'oauth_failure',
        provider: 'x',
    ...
    if (!sessionUser?.id) {
      logOAuthEvent({ event: 'oauth_failure', provider: 'x', callback_host: callbackHost, company_id: companyId ?? null, state_user_id: stateUserId ?? null, failure_point: 'unauthorized', failure_detail: 'no session' });
    ...
    if (stateUserId && stateUserId !== sessionUser.id) {
      logOAuthEvent({ event: 'oauth_failure', provider: 'x', callback_host: callbackHost, company_id: companyId ?? null, user_id: sessionUser.id, state_user_id: stateUserId, failure_point: 'invalid_oauth_state', failure_detail: 'state user does not match session user' });
      return res.redirect(`${errDest}?error=${encodeURIComponent('This connection was started by a different user - please try again')}`);
    }
    for (const orgId of new Set([companyId, stateTenantId].filter((v): v is string => Boolean(v)))) {
      const tenant = await assertTenantAccess({ userId: sessionUser.id, organizationId: orgId });
      if (tenant.ok !== true) {
        logOAuthEvent({ event: 'oauth_failure', provider: 'x', callback_host: callbackHost, company_id: orgId, user_id: sessionUser.id, state_user_id: stateUserId ?? null, failure_point: 'unauthorized', failure_detail: `company access denied (${tenant.reason})` });
