...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!sessionUser?.id) {
      logOAuthEvent({ event: 'oauth_failure', provider: 'pinterest', callback_host: callbackHost, company_id: companyId ?? null, state_user_id: stateUserId ?? null, failure_point: 'unauthorized', failure_detail: 'no session' });
    ...
    if (stateUserId && stateUserId !== sessionUser.id) {
      logOAuthEvent({ event: 'oauth_failure', provider: 'pinterest', callback_host: callbackHost, company_id: companyId ?? null, user_id: sessionUser.id, state_user_id: stateUserId, failure_point: 'invalid_oauth_state', failure_detail: 'state user does not match session user' });
    ...
    if (companyId) {
      ...
      if (tenant.ok !== true) {
        logOAuthEvent({ event: 'oauth_failure', provider: 'pinterest', callback_host: callbackHost, company_id: companyId, user_id: sessionUser.id, state_user_id: stateUserId ?? null, failure_point: 'unauthorized', failure_detail: `company access denied (${tenant.reason})` });
...
export default __createApiRoute(handler, { route: '/api/auth/pinterest/callback' });
