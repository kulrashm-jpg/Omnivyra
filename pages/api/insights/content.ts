...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const items = await runInApiReadContext('contentOpportunitiesApi', async () =>
      listDecisionFeatureView({
        viewName: 'content_opportunities_view',
    ...
    const message = err instanceof Error ? err.message : 'Failed to fetch content opportunities';
    return res.status(500).json({ error: message });
  }
}

export default __createApiRoute(withRBAC(handler, ALLOWED_ROLES), { route: '/api/insights/content' });
