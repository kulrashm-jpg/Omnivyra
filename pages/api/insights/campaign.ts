...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const items = await runInApiReadContext('campaignInsightsApi', async () =>
      listDecisionFeatureView({
        viewName: 'campaign_insights_view',
    ...
    const message = err instanceof Error ? err.message : 'Failed to fetch campaign insights';
    return res.status(500).json({ error: message });
  }
}

export default __createApiRoute(withRBAC(handler, ALLOWED_ROLES), { route: '/api/insights/campaign' });
