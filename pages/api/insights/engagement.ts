...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const items = await runInApiReadContext('engagementInsightsApi', async () =>
      listDecisionFeatureView({
        viewName: 'engagement_insights_view',
    ...
    const message = err instanceof Error ? err.message : 'Failed to fetch engagement insights';
    return res.status(500).json({ error: message });
  }
}

export default __createApiRoute(withRBAC(handler, ALLOWED_ROLES), { route: '/api/insights/engagement' });
