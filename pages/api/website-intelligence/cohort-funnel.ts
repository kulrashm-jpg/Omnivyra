...
import { buildCohortFunnelReport, type CohortKind } from '../../../backend/services/intelligence/cohortFunnelIntelligenceService';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!roleGate) return;
  const kindRaw = typeof req.query.kind === 'string' ? req.query.kind : 'session';
  const kind = (VALID_KINDS as string[]).includes(kindRaw) ? (kindRaw as CohortKind) : 'session';
