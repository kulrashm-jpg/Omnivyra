...
import { buildNextBestActions } from '../../../backend/services/intelligence/marketingNbaEngineService';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!roleGate) return;
  try { return res.status(200).json(await buildNextBestActions(companyId)); }
  catch (err) { return res.status(500).json({ error: err instanceof Error ? err.message : 'Failed' }); }
}

// W0-1 (Gate A): canonical route pipeline — pass-through observability + request context.
export default __createApiRoute(handler, { route: '/api/website-intelligence/next-best-action' });
