...
import { buildRepairPlan } from '../../../backend/services/intelligence/integrationRepairOrchestrationService';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!roleGate) return;
  try { return res.status(200).json(await buildRepairPlan(companyId)); }
  catch (err) { return res.status(500).json({ error: err instanceof Error ? err.message : 'Failed' }); }
}

// W0-1 (Gate A): canonical route pipeline — pass-through observability + request context.
export default __createApiRoute(handler, { route: '/api/website-intelligence/repair-plan' });
