...
import { buildCtaIntelligence } from '../../../backend/services/intelligence/marketingCtaIntelligenceService';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!roleGate) return;
  try { return res.status(200).json(await buildCtaIntelligence(companyId)); }
  catch (err) { return res.status(500).json({ error: err instanceof Error ? err.message : 'Failed' }); }
}

// W0-1 (Gate A): canonical route pipeline — pass-through observability + request context.
export default __createApiRoute(handler, { route: '/api/website-intelligence/cta-intelligence' });
