...
import { getIntelligenceOverview } from '../../../backend/services/intelligenceQueryService';
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const overview = await getIntelligenceOverview(companyId);
    ...
    return res.status(500).json({
      error: 'Failed to load intelligence overview. Please try again.',
    });
  }
}
