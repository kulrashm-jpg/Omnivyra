...
function deriveFallbackAction(score: number | null): 'continue' | 'optimize' | 'pivot' | null {
...
function deriveFallbackStability(history: Array<number | null>): 'stable' | 'sensitive' | 'volatile' | null {
...
function deriveFallbackConfidence(historyLength: number, score: number | null): string | null {
...
function derivePriority(action: 'continue' | 'optimize' | 'pivot', score: number | null, stability: string | null, confidence: string | null): 'high' | 'medium' | 'low' {
...
function buildAudienceMetricRankings(rows: Array<Record<string, unknown>>) {
  const metrics = [
...
function buildTopicClusters(records: CampaignRecord[]) {
...
function classifyCampaignPath(row: CampaignVersionRow): 'bolt_text' | 'bolt_creator' | 'intelligent_mix' | 'strategy_mix' | 'unknown' {
  const snapshot =
...
function integrationMatches(row: CompanyIntegrationRow, patterns: string[]): boolean {
  const haystack = [
    row.type,
    row.name,
...
function deriveReportMaturityStage(input: {
...
function deriveTimingThresholds(days: number): { strong: number; steady: number } {
  if (days <= 7) {
...
function mapContentTypeToStage(type: string): 'awareness' | 'consideration' | 'decision' {
