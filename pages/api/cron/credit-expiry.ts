 ...
 *
 * Daily expiry of time-based FREE/incentive credits (signup credits at their 30-day validity).
 * Delegates to creditExpiryService.runExpiryCheck() — idempotent per org per day; paid is never
 * expired (DB-enforced). Cron-only wiring; no balance/notification/ordering logic changes here.
 *
...
export default __createApiRoute(handler, { route: '/api/cron/credit-expiry' });
