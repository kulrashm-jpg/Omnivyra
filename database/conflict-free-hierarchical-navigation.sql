-- =====================================================
-- CONFLICT-FREE HIERARCHICAL NAVIGATION SYSTEM
-- =====================================================
-- Only adds new tables and safe extensions to existing structure

-- NEW TABLE 1: Weekly Content Refinements (No conflicts)
...
-- NEW TABLE 2: Daily Content Plans (No conflicts)
...
-- SAFE EXTENSION: Add only essential columns to existing content_plans
...
-- SAFE EXTENSION: Add weekly tracking to campaign_performance
ALTER TABLE campaign_performance
ADD COLUMN IF NOT EXISTS week_number INTEGER,
ADD COLUMN IF NOT EXISTS theme VARCHAR(255);

-- SAFE EXTENSION: Add plan review to ai_threads
...
CREATE INDEX IF NOT EXISTS idx_content_plans_refinement_status ON content_plans(refinement_status);

-- Function to get 12-week campaign overview (uses existing tables + new tables)
...
-- Success message
SELECT 'Conflict-free hierarchical navigation system created successfully! Safe to run.' as message;
