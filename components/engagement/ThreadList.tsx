...
export const ThreadList = React.memo(function ThreadList({
  ...
  if (loading) {
    return (
      <div className={`flex flex-col h-full ${className}`}>
        <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm p-3 border-b border-slate-200 font-medium text-slate-800">
          Threads
        </div>
  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm p-3 border-b border-slate-200 font-medium text-slate-800 flex items-center justify-between gap-2">
        <span>Threads</span>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {groupedAndSorted.map(({ group, threads }) => (
          <div key={group}>
            ...
            {threads.map((thread) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-start gap-2 w-full text-left p-3 border-b border-slate-100 hover:bg-slate-50 transition-colors border-l-4 ${priorityBorder} ${isSelected ? 'bg-blue-50 shadow-[inset_0_0_0_1px_rgba(59,130,246,0.08)]' : ''}`}
                >
                  {triageDotColor && (
                    <span
