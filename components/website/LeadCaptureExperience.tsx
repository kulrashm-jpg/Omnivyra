...
/** Route each lead-capture intent's page lives on (for canonical/og:url). */
const INTENT_PATHS: Partial<Record<LeadIntent, string>> = {
...
export default function LeadCaptureExperience({ intent }: { intent: LeadIntent }) {
  ...
  return (
    <>
      <MarketingPageMeta
        title={`${cfg.cta} | OmniVyra`}
        description={cfg.subheading}
        path={INTENT_PATHS[intent] ?? '/contact-sales'}
      />
