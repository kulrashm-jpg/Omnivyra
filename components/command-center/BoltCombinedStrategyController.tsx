/** useBoltCombinedStrategyController — prelude + state/handlers, verbatim (guards → flags). */
/**
...
export const TEXT_FORMATS: { value: TextFormat; label: string; icon: string; hint: string }[] = [
...
export const CREATOR_FORMATS: { value: CreatorFormat; label: string; icon: string; hint: string }[] = [
...
export const DURATION_OPTIONS = COMBINED_DURATION_OPTIONS;
...
export const AUDIENCE_OPTIONS = getSelectableAudienceLabels({ includeGroups: ['professional', 'industry'] });
...
/* ─── Strategy Card ─────────────────────────────────────────────────────────── */
export function StrategyCard({ card, index, selected, boltProgress, execStartedAt, anyExecuting, onSelect }: {
...
/* ─── Main page ─────────────────────────────────────────────────────────────── */
export function useBoltCombinedStrategyController() {
  ...
  // Guards moved to the render layer (a hook must not return JSX).
  const guardLoading = !authChecked || isLoading;
  const guardRedirect = !user?.userId;
