...
import { deriveCreatorStatusLabel, isCreatorProduced } from '../../lib/shared/creatorStatusLabel';
...
/** Status chip used on the day-detail card (label + AI/Creator marker). */
function CalendarStatusChip({ event }: { event: ActivityEvent }) {
  ...
  return (
    <span className="inline-flex items-center gap-1">
      <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${chip}`}>{label}</span>
      <span
        className={`px-1.5 py-0.5 rounded-full text-[9px] font-semibold ${creator ? 'bg-indigo-50 text-indigo-600' : 'bg-sky-50 text-sky-600'}`}
        title={creator ? 'Creator-produced — you upload the video' : 'AI-generated by Omnivyra'}
      >
        {creator ? 'Creator' : 'AI'}
      </span>
    </span>
                            ...
                            if (isActivityEvent(item)) {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {/* Compact status dot — full Platform • Type • Status in the tooltip + drawer. */}
                                  <span
                                    className={`shrink-0 w-1.5 h-1.5 rounded-full ${STATUS_DOT_CLASS[deriveCreatorStatusLabel(item).group] ?? 'bg-gray-400'}`}
                                  />
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <div className="min-w-0 flex-1">
                                  <p className="font-medium text-gray-900 truncate capitalize">{getPlatformLabel(activity.platform)} • {resolveDisplayContentTypeLabel(activity)}</p>
                                  <p className="text-sm text-gray-700 truncate">{activity.title}</p>
                                  <div className="flex items-center gap-2 mt-1 text-xs text-gray-500 flex-wrap">
                                    <CalendarStatusChip event={activity} />
                                    {<RepurposeDots index={activity.repurpose_index} total={activity.repurpose_total} contentType={resolveDisplayContentType(activity)} />}
