...
const CampaignProgress: React.FC<{ campaignId: string; companyId?: string | null }> = ({
  ...
  const [progress, setProgress] = useState<CampaignProgressData>({
    percentage: 0,
    contentCount: 0,
    scheduledCount: 0,
    publishedCount: 0
  ...
  useEffect(() => {
    const loadProgress = async () => {
      ...
      } catch { /* keep defaults */ } finally { setIsLoadingProgress(false); }
    };
    loadProgress();
  ...
  const progressColor = percentage === 0 ? 'bg-gray-400' : percentage < 30 ? 'bg-red-500' : percentage < 70 ? 'bg-yellow-500' : 'bg-green-500';
  return (
    <div className="flex items-center">
      <div className="w-16 bg-gray-200 rounded-full h-2 mr-2">
        <div className={`h-2 rounded-full transition-all duration-300 ${progressColor}`} style={{ width: `${Math.max(percentage, 5)}%` }}></div>
      </div>
