...
import {
  buildRegenerationReadinessContract,
  serializeRegenerationReadinessContract,
} from '../../../lib/content/regenerationReadinessContracts';
...
function buildContractInput(contentHtml: string) {
  ...
  const generatedContent = {
    ...
    key_insights: ['Regeneration contracts must preserve workflow proof and reader-state movement.'],
  };
...
describe('regenerationReadinessContracts', () => {
  it('generates deterministic regeneration contracts', () => {
    const first = buildRegenerationReadinessContract(buildContractInput(alignedHtml));
    const second = buildRegenerationReadinessContract(buildContractInput(alignedHtml));

    expect(JSON.stringify(first)).toBe(JSON.stringify(second));
    expect(first.version).toBe('regeneration-readiness-contracts-v1');
    expect(first.safeRecoveryBoundaries.length).toBeGreaterThan(0);
  });

  it('generates rewrite boundaries from section recovery targets', () => {
    const contract = buildRegenerationReadinessContract(buildContractInput(`
...
`));
    ...
    expect(contract.sectionRewriteTargets.length).toBeGreaterThan(0);
    expect(contract.sectionRewriteTargets[0].rewriteBoundaries.length).toBeGreaterThan(0);
    expect(contract.sectionRewriteBoundaries[String(contract.sectionRewriteTargets[0].sectionIndex)].length).toBeGreaterThan(0);
  });

  it('maps preservation constraints', () => {
    const contract = buildRegenerationReadinessContract(buildContractInput(alignedHtml));

    expect(contract.preservationConstraints.join(' ')).toContain('narrative progression');
    expect(contract.sectionPreservationRequirements[String(contract.sectionRewriteTargets[0].sectionIndex)].join(' ')).toContain('progression stage');
  });

  it('targets eligible sections for future rewrites', () => {
    const contract = buildRegenerationReadinessContract(buildContractInput(`
...
`));
    ...
    expect(contract.regenerationEligibility).toBe('eligible');
    expect(contract.sectionRewriteTargets.some((section) => section.eligibility === 'eligible')).toBe(true);
  });

  it('maps narrative and anti-repetition preservation requirements', () => {
    const contract = buildRegenerationReadinessContract(buildContractInput(alignedHtml));

    expect(contract.narrativePreservationRequirements.join(' ')).toContain('diagnose');
    expect(contract.antiRepetitionRecoveryRequirements.join(' ')).toContain('repeated');
  });

  it('maps authority preservation requirements', () => {
    const contract = buildRegenerationReadinessContract(buildContractInput(alignedHtml));

    expect(contract.authorityPreservationRequirements.join(' ')).toContain('invent');
    expect(contract.authorityPreservationRequirements.join(' ')).toContain('evidence');
  });

  it('serializes compact regeneration contracts', () => {
    const contract = buildRegenerationReadinessContract(buildContractInput(alignedHtml));
    const serialized = serializeRegenerationReadinessContract(contract);

    expect(serialized).toContain('## REGENERATION READINESS CONTRACTS');
    expect(serialized).toContain('Eligibility:');
    expect(serialized).toContain('Section rewrite targets:');
