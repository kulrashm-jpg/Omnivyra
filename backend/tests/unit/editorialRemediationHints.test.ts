...
import {
  buildEditorialRemediationHints,
  serializeEditorialRemediationHints,
} from '../../../lib/content/editorialRemediationHints';
...
function buildRemediationInput(contentHtml: string) {
  ...
  const generatedContent = {
    ...
    key_insights: ['Remediation should preserve workflow proof and reader-state movement.'],
  };
...
describe('editorialRemediationHints', () => {
  it('generates deterministic remediation hints', () => {
    const first = buildEditorialRemediationHints(buildRemediationInput(alignedHtml));
    const second = buildEditorialRemediationHints(buildRemediationInput(alignedHtml));

    expect(JSON.stringify(first)).toBe(JSON.stringify(second));
    expect(first.version).toBe('editorial-remediation-hints-v1');
    expect(first.remediationPriority).toBeTruthy();
  });

  it('maps risks into remediation hints', () => {
    const report = buildEditorialRemediationHints(buildRemediationInput(`
...
`));
    ...
    expect(report.editorialRiskRemediationHints.length).toBeGreaterThan(0);
    expect(report.editorialRiskRemediationHints[0].sourceRisks.length).toBeGreaterThan(0);
    expect(['critical', 'high', 'medium']).toContain(report.remediationPriority);
  });

  it('aggregates section remediation context', () => {
    const report = buildEditorialRemediationHints(buildRemediationInput(alignedHtml));

    expect(report.sectionRemediationHints.length).toBeGreaterThan(0);
    expect(report.sectionRemediationHints.every((section) => section.remediationTargets.length > 0)).toBe(true);
  });

  it('normalizes remediation priority from readiness tiers', () => {
    const report = buildEditorialRemediationHints(buildRemediationInput(`
...
`));
    ...
    expect(report.antiRepetitionRemediationHints[0].priority).toBe('critical');
    expect(report.remediationPriority).toBe('critical');
  });

  it('maps behavioral remediation signals', () => {
    const report = buildEditorialRemediationHints(buildRemediationInput(alignedHtml));

    expect(report.behavioralConsistencyRemediationHints.length).toBeGreaterThan(0);
    expect(report.behavioralConsistencyRemediationHints[0].correctionSignals.join(' ')).toContain('primary behavior');
  });

  it('maps anti-repetition and authority remediation', () => {
    const report = buildEditorialRemediationHints(buildRemediationInput(`
<h2>A</h2><p>This section gives an overview of best practices and innovative ways to drive growth with generic recommendations.</p>
<h2>B</h2><p>This section gives an overview of best practices and innovative ways to drive growth with generic recommendations.</p>
<h2>C</h2><p>This section gives an overview of best practices and innovative ways to drive growth with generic recommendations.</p>
`));
    ...
    expect(report.antiRepetitionRemediationHints[0].avoidSignals.join(' ')).toContain('repeated');
    expect(report.authorityRemediationHints[0].remediationIntent).toContain('evidence');
  });

  it('serializes compact remediation context', () => {
    const report = buildEditorialRemediationHints(buildRemediationInput(alignedHtml));
    const serialized = serializeEditorialRemediationHints(report);

    expect(serialized).toContain('## EDITORIAL REMEDIATION HINTS');
    expect(serialized).toContain('Remediation priority:');
    expect(serialized).toContain('Dimension hints:');
