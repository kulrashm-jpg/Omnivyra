...
jest.mock('../../services/leadIngestion/orchestrator', () => ({
  ...
  // Tenant-scoped gate. Defaults ALLOW so this suite keeps testing the route's
  // authorization ordering; the gate's own deny paths are proven in
  // piTenantScopedIngestionGate.test.ts.
...
beforeEach(() => {
  jest.clearAllMocks();
  ingestionEnabled = true;                                     // enabled by default
  enforceCompanyAccess.mockResolvedValue({ userId: 'u-1' });   // a member by default
  requireCapability.mockResolvedValue({ ok: true, principal: { userId: 'u-1' } });
  ingestLeadBatch.mockResolvedValue(OK_RESULT);
});

describe('LI-5E.2 — the capability gate', () => {
  it('disabled: 404 with exactly the two-field disabled contract', async () => {
    ...
    // Exactly two fields — no `disabled`, no `retryable`, no `message`.
  ...
  it('disabled: refused before the tenant is even parsed — a malformed company_id is still 404', async () => {
    ingestionEnabled = false;
    const res = await call({ query: { company_id: 'not-a-uuid' } });
    expect(res._status).toBe(404);
...
describe('LI-5E.2 — capability authorization', () => {
  it('requires prospect.ingest, bound to the VERIFIED tenant', async () => {
    await call();
    ...
    expect(opts.organizationId).toBe(ORG_A);      // the query tenant, membership-verified
  ...
  it('a member WITHOUT the capability is refused, and never reaches ingestion', async () => {
    ...
    expect(ingestLeadBatch).not.toHaveBeenCalled();
    expect(trackEvent).not.toHaveBeenCalled();     // an unadmitted batch emits nothing
  });

  it('binds to the query tenant even when the body names another — the body is never authority', async () => {
    // The body override is refused later with 400, but authorization has already
    // been decided against the verified tenant, never the claimed one.
...
describe('LI-5E.2 — gate ordering', () => {
  ...
  it('a membership failure stays a membership failure — capability is never consulted', async () => {
    enforceCompanyAccess.mockResolvedValue(null);   // writes its own 403 and returns null
    await call();
    expect(requireCapability).not.toHaveBeenCalled();
    expect(ingestLeadBatch).not.toHaveBeenCalled();
  });
  ...
  it('capability is checked only AFTER membership succeeds', async () => {
    await call();
    const order = [
      enforceCompanyAccess.mock.invocationCallOrder[0],
      requireCapability.mock.invocationCallOrder[0],
    ];
    expect(order[0]).toBeLessThan(order[1]);
  });
});

describe('LI-5E.2 — durable batch telemetry', () => {
  ...
  it('carries the verified tenant, the actor, the source and the counts', async () => {
    await call();
    ...
    expect(e.metadata).toEqual({
      source: OK_RESULT.source,
      total: OK_RESULT.total,
      succeeded: OK_RESULT.succeeded,
      failed: OK_RESULT.failed,
    });
  });

  it('dedupes on the ingestionRunId when the caller supplied one', async () => {
    await call({ body: { records: [{ referenceId: 'OP-1' }], ingestionRunId: 'run-7' } });
    const e = trackEvent.mock.calls[0][0] as Record<string, unknown>;
    expect(e.entityId).toBe('run-7');
    expect(e.dedupKey).toBe('lead.ingestion_batch:run-7');
  });

  it('without an ingestionRunId the event is not deduplicated', async () => {
    await call();
    const e = trackEvent.mock.calls[0][0] as Record<string, unknown>;
    expect(e.entityId).toBeNull();
    expect(e.dedupKey).toBeNull();
  });

  it('emits NO PII — counts, a source constant and identifiers only', async () => {
    await call({ body: { records: [{ referenceId: 'OP-1', email: 'a@x.test', name: 'A Person' }] } });
    const blob = JSON.stringify(trackEvent.mock.calls[0][0]);
    expect(blob).not.toContain('a@x.test');
    expect(blob).not.toContain('A Person');
    expect(blob).not.toContain('records');
    // The metadata surface is exactly the four declared keys.
