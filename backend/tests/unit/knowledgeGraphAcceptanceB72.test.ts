...
describe('B7.2 · flag ON ⇒ the graph path executes', () => {
  ...
  /**
   * B7.10 — campaign attribution is DEFERRED, and the change is recorded here
   * rather than made silently.
   *
   * These two cases originally asserted that coverage preserves campaign_id.
   * On origin/main `CreateContentInput` carries no `campaignId` and the content
   * row has no `campaign_id` column, so the producer has no honest source for
   * one. Supplying it would mean changing the content contract — outside this
   * producer's scope. The assertions therefore lock in what is actually true
   * today: nothing is invented. `company_topic_coverage.campaign_id` remains a
   * nullable soft ref awaiting a caller that legitimately has a campaign.
   */
  it('never invents a campaign — the content contract carries none', async () => {
    await createContent(req());
    expect(mockRecordCoverage.mock.calls[0][0].campaignId).toBeUndefined();
  });

  it('passes no campaign key at all, rather than a fabricated null', async () => {
    await createContent(req());
    expect('campaignId' in mockRecordCoverage.mock.calls[0][0]).toBe(false);
  });
...
describe('B7.2 · does not alter existing content behaviour', () => {
  ...
  it('the returned artifact shape is identical with the flag ON and OFF', async () => {
    mockEnabled.mockReturnValue(false);
    const off = await createContent(req());
    installSupabase();
    mockEnabled.mockReturnValue(true);
    const on = await createContent(req());
    // The key-set comparison is the real invariant: enrichment must not change
    // the artifact the caller receives. (`campaignId` is not on
    // CanonicalContent, so the original per-field check could not compile.)
