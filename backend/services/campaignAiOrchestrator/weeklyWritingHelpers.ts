...
export function deterministicFormatRequirements(contentTypeRaw: string): {
  ...
  const isPost = ct.includes('post') || ct.includes('feed_post') || ct.includes('tweet') || ct === 'post';
...
export function validateContentTypeFormatPlatform(input: {
  ...
  if (hardMismatch) {
    if (allowedFamilies.has(inferred)) return { content_type, format_family: inferred, format_validation_warning: false };
