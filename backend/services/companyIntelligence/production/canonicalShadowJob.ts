 ...
 */
export function makeSupabaseShadowDeps(sb: {
  from: (table: string) => {
  ...
  return {
    readReportSettings: async (companyId) => {
      const { data } = await sb.from('company_profiles').select('report_settings').eq('company_id', companyId).maybeSingle();
      return (data?.report_settings as Record<string, unknown> | null) ?? null;
    },
    writeReportSettings: async (companyId, reportSettings) => {
      await sb.from('company_profiles').update({ report_settings: reportSettings }).eq('company_id', companyId);
    },
  };
}
