 ...
 *
 * SEC91-D6 — `credentialScope`: the exact key (and the gateway's in-flight
 * coalescing key, which is this same value) used to omit WHOSE credential
 * produced the answer. A company on its own key (BYOK) could therefore be
 * served a response generated on another tenant's key — or have its own key
 * pay for (coalesce) another tenant's identical request. The gateway passes a
 * scope (`byok:<companyId>`) for BYOK calls so those entries are reachable
 * only by that company. Absent/empty scope ⇒ the payload, and so the key, is
 * byte-identical to before (platform-key entries and the near-match v3 index,
 * which stores exact keys, are unchanged).
 */
export function buildNormalizedKey(
  ...
  const payload = credentialScope
    ? JSON.stringify({ model, messages: normalized, v: cacheVersion ?? '', s: credentialScope })
    : JSON.stringify({ model, messages: normalized, v: cacheVersion ?? '' });
...
export async function getCachedCompletion(
  ...
  /** SEC91-D6: credential scope (see buildNormalizedKey). */
  credentialScope?: string | null,
): Promise<string | null> {
  ...
  try {
    // ── Exact match (GAP 1 + 5) ──────────────────────────────────────────────
    const exactKey = buildNormalizedKey(model, messages, cacheVersion, credentialScope);
    ...
    for (const raw of rawEntries) {
      try {
        const entry = JSON.parse(raw) as {
          words: string[]; key: string; t?: string; m?: string; v?: string; s?: string;
        };
        ...
        // SEC91-D6: agree on credential scope too. Entries written before this
        // change carry no `s` and are platform-scope (''), so platform lookups
        // behave exactly as before.
        if ((entry.s ?? '') !== (credentialScope ?? '')) continue;
...
export async function setCachedCompletion(
  ...
  try {
    ...
    const entry = JSON.stringify({
      ...
      // SEC91-D6: only written when scoped, so platform entries are unchanged.
