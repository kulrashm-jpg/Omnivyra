/**
 * Deterministic cohort funnel engine — REPLACES stage-only funneling with
 * per-cohort continuity. Cohorts are defined by *origin signal*, not by stage:
 *
 *   - per-session cohort      (visitor_session_id)
 *   - per-user cohort         (lead_id, or email_hash when ambient)
 *   - per-domain cohort       (host of first touchpoint URL)
 *   - per-campaign cohort     (campaign + source + medium combo)
 *
 * Each lead is mapped into its cohort and traced through stages:
 *   visit → engage → lead → opportunity → closed_won
 *
 * The same stage set is computed for revenue cohorts when revenue events
 * (audit_events:crm_revenue_event) carry an attribution_token nonce that ties
 * to a lead.
 *
 * Everything is deterministic + explainable:
 *   - cohorts come from rows that already exist (no synthesized membership)
 *   - dropoff = count change between adjacent stages
 *   - attribution-break = leads in cohort with zero touchpoints
 *   - confidence = monotonic in cohort size (sample-size only, not ML)
 */
...
async function load(companyId: string, sinceIso: string): Promise<{ touches: TouchpointRow[]; leads: LeadRow[]; sessions: SessionRow[]; revenues: RevenueRow[] }> {
  ...
  return { touches, leads, sessions, revenues };
}
...
export async function buildCohortFunnelReport(companyId: string, kind: CohortKind = 'session'): Promise<CohortFunnelReport> {
  const sinceIso = new Date(Date.now() - WINDOW_DAYS * 86_400_000).toISOString();
  const { touches, leads, sessions, revenues } = await load(companyId, sinceIso);
