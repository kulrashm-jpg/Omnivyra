 ...
 *
 * Applies time-based decay to campaign_autonomous_learnings so recent wins are weighted
...
export async function reinforceLearning(learningId: string, impactBoost: number = 0.1): Promise<void> {
  const { data } = await ownedDbTable('campaign_autonomous_learnings')
  ...
  await ownedDbTable('campaign_autonomous_learnings').update({
...
/**
 * Run decay pass over all campaign_autonomous_learnings.
...
export async function runLearningDecay(companyId?: string): Promise<DecayRunResult> {
  ...
  try {
    let query = ownedDbTable('campaign_autonomous_learnings')
    ...
    for (const row of learnings as Array<{
      ...
      try {
        ...
        if (effectiveScore < PRUNE_THRESHOLD && row.reinforcement_score < 0.1) {
          ...
          await ownedDbTable('campaign_autonomous_learnings').delete().eq('id', row.id);
          result.pruned++;
        } else {
          await ownedDbTable('campaign_autonomous_learnings')
