...
// primitive for creator assets (duplicate slides/sections cannot persist).
...
/**
 * CAMPAIGN-IMPL-007A: campaign-scoped semantic-validation contexts. Keyed by
 * campaignId so creator assets generated by the same worker share a context
 * (cross-asset duplicate detection within a run); within-asset checks (duplicate
 * slides/sections, master-idea consistency) work regardless. In-memory + bounded.
 */
const creatorValidationContexts = new Map<string, ValidationContext>();
function getCreatorValidationContext(campaignId: string): ValidationContext {
  const key = campaignId || 'no-campaign';
  let ctx = creatorValidationContexts.get(key);
  if (!ctx) {
    ctx = new ValidationContext();
    // Bound the registry so a long-lived worker can't grow unbounded.
    if (creatorValidationContexts.size > 200) creatorValidationContexts.clear();
    creatorValidationContexts.set(key, ctx);
  }
  return ctx;
}
...
export async function runCreatorOrchestration(
  ...
  // 2d) CAMPAIGN-IMPL-007A — SEMANTIC VALIDATION GATE (before render + persist).
  //     Reuse the ONE engine: validate the structured asset (duplicate slides /
  //     sections / CTA / idea / narrative / master-idea consistency). On a fixable
  //     duplicate, regenerate via the shared regenerateBeforeDrop primitive; if
  //     still failing, throw — mirroring the generation-validation failure above,
  //     so a duplicate creator asset can NEVER persist. Fail-safe wrapper.
  try {
    const valCtx = getCreatorValidationContext(String(input.campaignId ?? ''));
    const CREATOR_REGEN_ATTEMPTS = Math.max(0, Math.round(Number(process.env.CREATOR_VALIDATION_REGEN_ATTEMPTS ?? 1) || 0));
    const assetOf = (ready: CanonicalCreatorOutput) => creatorAssetToGenerated({
      content_type: effectiveContentType,
      platform: primaryPlatform,
      asset_payload: safeObject((ready as unknown as Record<string, unknown>).asset_payload),
      content: safeObject(input.existingContent),
    });
    let verdict = validateAsset(assetOf(renderReady), valCtx);
    let creatorRegenerated = false;
    if (verdict.decision === 'REGENERATE' && CREATOR_REGEN_ATTEMPTS > 0) {
      const outcome = await regenerateBeforeDrop<CanonicalCreatorOutput>(
        async () => {
          try {
            const regen = await (engine as any).generateFromIntent(generationCtx, { companyId: input.companyId }, { assetOverride: safeObject(safeObject(input.existingContent).asset_payload || safeObject(input.existingContent).creator_asset) }) as CanonicalCreatorOutput;
            const merged = input.appliedVariant ? mergeAppliedVariantIntoOutput(regen, input.appliedVariant) : regen;
            return await engine.adaptForPlatform(merged, primaryPlatform) as CanonicalCreatorOutput;
          } catch { return null; }
        },
        (cand) => { const d = validateAsset(assetOf(cand), valCtx).decision; return d === 'ACCEPT' || d === 'ADAPT'; },
        CREATOR_REGEN_ATTEMPTS,
      );
      if (outcome.result) { renderReady = outcome.result; creatorRegenerated = outcome.regenerated; verdict = validateAsset(assetOf(renderReady), valCtx); }
    }
    try {
      const ct = effectiveContentType;
      if (creatorRegenerated) recordRawCounter('creator.validation.regenerated', 1, { content_type: ct });
      if (verdict.decision === 'DROP' || verdict.decision === 'REGENERATE') recordRawCounter('creator.validation.dropped', 1, { content_type: ct });
      else recordRawCounter('creator.validation.accepted', 1, { content_type: ct });
      if (verdict.findings[0]) recordRawCounter('creator.validation.reason', 1, { reason: verdict.findings[0].dimension });
    } catch { /* observability only */ }
    if (verdict.decision === 'DROP' || verdict.decision === 'REGENERATE') {
      throw new Error(`creator orchestrator: semantic validation failed (${verdict.reason}) — dropped to prevent a duplicate asset`);
    }
    valCtx.commit(assetOf(renderReady));
  } catch (err) {
    // A validation-triggered drop must propagate (it prevents a bad persist); a
    // fault in the gate itself must NOT block generation.
    if (err instanceof Error && err.message.startsWith('creator orchestrator: semantic validation failed')) throw err;
    console.warn('[creator-orchestrator][semantic-validation] non-fatal gate error', { message: (err as Error)?.message });
  }
