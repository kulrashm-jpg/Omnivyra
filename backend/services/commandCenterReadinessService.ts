...
export function getCardStateFromFeatures(
  ...
  // K2: absent is not the same fact as incomplete.
  //
  // completedCount below counts only the keys the dataset actually CONTAINS, but
  // is compared against the full required count — so a dataset missing keys can
  // never reach `ready`, and an empty one reads as `not_started`. Both silently
  // present "we could not load this" as "you have not set this up".
  //
  // A key that is PRESENT and merely incomplete is real product state and still
  // flows through the unchanged logic below.
