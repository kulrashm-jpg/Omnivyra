...
// Visual formats (carousel / image / infographic / banner) are AI-generated by the
// creator renderer — no user template required — so they classify as AI_AUTOMATED,
// not CONDITIONAL_AI. CONDITIONAL_AI now applies only to genuinely unknown types
// (the safe default below), not to any known visual format.
...
const AI_AUTOMATED_TYPES = new Set([
  ...
  // Visual formats — AI generates these via the creator renderer (no template needed).
 ...
 * - carousel, slides, infographic, deck, image, banner → AI_AUTOMATED (creator renderer)
 * - text, post, article, thread, story, tweet, blog → AI_AUTOMATED
 * - only genuinely unknown content types fall back to CONDITIONAL_AI
...
export function inferExecutionMode(
  ...
  // Visual formats are AI-generated (creator renderer) — AI_AUTOMATED, not CONDITIONAL.
