/**
 * Platform allocation advice for a campaign — the computation behind
 * GET /api/campaigns/[id]/platform-allocation-advice.
 *
 * Extracted from the route (STEP 3AH-91 SEC-E1) so that
 * POST /api/campaigns/[id]/propose-frequency-rebalance can use it in-process.
 * That route previously obtained the advice with a server-side `fetch` to the
 * caller-supplied `Origin` header, forwarding the caller's Authorization and
 * Cookie headers — an SSRF that also handed the caller's credentials to any
 * host the caller named. There is now no HTTP hop at all.
 *
 * AUTHORIZATION IS THE CALLER'S JOB: this function reads by campaign id only.
 * Both routes authenticate the user and prove membership in the campaign's own
 * company (derived from `campaign_versions`) before calling it.
 */
import { supabase } from '../db/supabaseClient';

export interface PlatformAdviceItem {
  platform: string;
  allocation_score: number;
  recommendation: 'Increase' | 'Maintain' | 'Reduce';
  rationale: string;
  suggested_frequency_delta: number;
}

export interface PlatformAllocationAdvice {
  platform_advice: PlatformAdviceItem[];
...
export async function computePlatformAllocationAdvice(campaignId: string): Promise<PlatformAllocationAdvice> {
  const id = campaignId;
  ...
  const platform_advice: PlatformAdviceItem[] = platforms.map((platform) => {
    ...
    const recommendation: PlatformAdviceItem['recommendation'] =
      allocationScore >= 0.7
        ? 'Increase'
  ...
  return { platform_advice, reallocation_summary };
}
