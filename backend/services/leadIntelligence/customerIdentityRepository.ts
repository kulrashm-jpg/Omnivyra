/**
 * Canonical Lead Intelligence Repository — customer-identity continuity (identity graph).
 *
 * Owns the data-acquisition layer for the deterministic identity-stitching engine:
 * the visitor_sessions / leads / campaign_touchpoints / audit_events reads
 * (relocated verbatim, fail-open, windowed). The graph analysis (`analyzeIdentity`)
 * — Union-Find, the six deterministic edge rules (SESSION_TO_LEAD, EMAIL_HASH,
 * CROSS_DOMAIN, CROSS_DEVICE, REPEAT_VISITOR, WEBHOOK_IDENTITY), salted email
 * hashing, cluster building, drift flags, confidence — is moved unchanged and is
 * pure over the hydrated inputs. The best-effort compliance-audit snapshot stays in
 * the orchestrator so behaviour (one write per report) is identical.
 *
 * Only the sources the engine consumes are hydrated; unified_persons / identity
 * mapping tables are NOT read by this engine and are intentionally not collected.
 *
...
export interface CustomerIdentityInputs {
  companyId: string;
  sessions: SessionRow[];
  leads: LeadRow[];
  touches: TouchRow[];
  revenues: RevenueAuditRow[];
}
...
export async function getCustomerIdentityInputs(
  companyId: string,
  now: number = Date.now(),
): Promise<CustomerIdentityInputs> {
  ...
  return { companyId, sessions, leads, touches, revenues };
}

/** Pure identity-graph analysis over hydrated inputs — moved verbatim from the engine. */
export function analyzeIdentity(inputs: CustomerIdentityInputs, now: number): IdentityContinuityReport {
  const { companyId, sessions, leads, touches, revenues } = inputs;
...
export async function getCustomerIdentityContinuity(
  companyId: string,
  now: number = Date.now(),
): Promise<IdentityContinuityReport> {
  const inputs = await getCustomerIdentityInputs(companyId, now);
  const report = analyzeIdentity(inputs, now);

  // Append a single aggregate lineage row so the report is replayable.
  try {
    const aggResourceId = `identity_continuity:${companyId}:${new Date(now).toISOString().slice(0, 10)}`;
    await recordComplianceAudit({
      ...
      detail: {
        ...
        multiSignalClusters: report.clusters.filter((c) => c.confidence >= 40).length,
        driftFlags: report.driftFlags.length,
      },
    });
  } catch { /* best-effort */ }
