/**
 * workspaceContentPrompt.ts — the ONE prompt source for the workspace Content
 * Writer (PMF-001 §6/§8).
 *
 * VERBATIM extraction of the prompt/spec logic that previously lived inline in
 * pages/api/planner/generate-workspace-content.ts. Both the legacy route branch
 * and the migrated AIC capability import from here, so there is exactly ONE
 * prompt-assembly source (no duplicate prompt assembly). The strings are moved
 * unchanged — generation behavior is preserved byte-for-byte.
 */
...
export const CONTENT_TYPE_GUIDANCE: Record<string, string> = {
...
export const WORKSPACE_SYSTEM_PROMPT = `You are an expert social media content strategist and copywriter. You write platform-native content — each piece feels like it was created specifically for that platform by someone who lives on it.
...
/** Build the per-platform instruction blocks — verbatim. */
export function buildPlatformBlocks(platforms: string[], contentTypes?: Record<string, string>): string {
  return platforms.map((p) => {
    const spec = getPlatformSpec(p);
    const ct = (contentTypes?.[p] ?? 'post').toLowerCase();
...
/** Build the campaign context lines — verbatim. */
export function buildContextLines(opts: { theme?: string; objective?: string; week?: string | number }): string[] {
  const contextLines: string[] = [];
  if (opts.theme) contextLines.push(`Weekly theme: ${opts.theme}`);
  if (opts.objective) contextLines.push(`Objective: ${opts.objective}`);
  if (opts.week) contextLines.push(`Campaign week: ${opts.week}`);
  return contextLines;
}
...
Return JSON: { "${opts.platforms.map((p) => p.toLowerCase()).join('": "...", "')}" : "..." }`;
}
