...
/**
 * CANONICAL CREDIT TYPES — re-exported, NOT redefined here.
 *
 * The authoritative definitions live in `creditReadService.ts`. These
 * re-exports exist only so historical/dynamic import sites that referenced
 * `from './consumptionAnalyticsService'` keep resolving to the one true
 * shape. Do NOT reintroduce a local copy — a duplicated interface is exactly
 * how the silent-0 shadow reader drifted in originally.
 */
export type { CreditTransaction, OrgCreditSummary } from './creditReadService';
...
/**
 * CANONICAL, RECONCILIATION-AWARE org credit summary.
 *
 * This module MUST NOT host its own implementation. The former local copy
 * bypassed the Phase-D projection self-heal and silently returned
 * zeroed/`null` balances. It is now a pure re-export so every caller — past
 * or future — resolves to the hardened reader in `creditReadService`
 * (gated wallet reconciliation + explicit failure, no silent 0).
 */
export { getOrgCreditSummary } from './creditReadService';
