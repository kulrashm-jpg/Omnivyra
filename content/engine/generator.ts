...
import { runCompletionWithOperation as runGatewayCompletionWithOperation } from '../../backend/services/aiGateway';
import { getStructureRules, getArticleStructureRules, getWhitepaperStructureRules, getNewsletterStructureRules, getStoryStructureRules, getGuideStructureRules, isValidArticleFormat, isValidWhitepaperFormat, isValidNewsletterFormat, isValidStoryFormat, isValidGuideFormat, type BlogFormatType, type ArticleFormatType, type WhitepaperFormatType, type NewsletterFormatType, type StoryFormatType, type GuideFormatType } from '../../lib/blog/blogStructureTemplates';
import { assertContentType, type ContentType } from '../core/contentRegistry';
import {
...
type ContentAIRequest = Parameters<typeof runGatewayCompletionWithOperation>[0];
type ContentAIResponse = Awaited<ReturnType<typeof runGatewayCompletionWithOperation>>;

const PROMPT_CACHE = new Map<string, string>();
const AI_OUTPUT_CACHE = new Map<string, { output: string; expiresAt: number }>();
const AI_OUTPUT_TTL_MS = 5 * 60 * 1000;

function cacheKey(value: unknown): string {
  return createHash('sha256').update(JSON.stringify(value)).digest('hex');
}

export async function runCompletionWithOperation(request: ContentAIRequest): Promise<ContentAIResponse> {
  return runGatewayCompletionWithOperation(request);
}
...
export function compilePrompt(type: ContentType, template: string | undefined, input: GenerateParams['input']): string {
  assertContentType(type);
  const key = cacheKey({ type, template, input });
  ...
  const compiled = [
    `Create ${type} content as structured, publication-ready HTML.`,
    'Do not include scripts, inline event handlers, or unsafe HTML.',
    'The formatter will convert this HTML into canonical content blocks.',
    template ? `Template: ${template}` : '',
    `Input:\n${normalizedInput}`,
  ].filter(Boolean).join('\n\n');

  PROMPT_CACHE.set(key, compiled);
  return compiled;
}
...
export async function generateContent({
  ...
  const compiledPrompt = compilePrompt(type, template, input);
  const outputKey = cacheKey({ type, template, input, companyId, model: process.env.OPENAI_MODEL || 'gpt-4o-mini' });
  const cached = AI_OUTPUT_CACHE.get(outputKey);
  if (cached && cached.expiresAt > Date.now()) {
    return cached.output;
  }

  const aiOutput = await runCompletionWithOperation({
    companyId,
    ...
    messages: [
      {
        role: 'system',
        content: 'You are the unified content generation engine. Return only safe HTML body content.',
      },
      {
        role: 'user',
  ...
  AI_OUTPUT_CACHE.set(outputKey, {
    output: aiOutput.output,
    expiresAt: Date.now() + AI_OUTPUT_TTL_MS,
  });
  return aiOutput.output;
}
