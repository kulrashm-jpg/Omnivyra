 ...
 *   buildTemplateAwareSystemPrompt
 *   buildTemplateAwareSystemPromptV2
 *   buildTemplateAwareUserPrompt
 */

import {
  getStructureRules,
  getArticleStructureRules,
  getWhitepaperStructureRules,
  getNewsletterStructureRules,
  getStoryStructureRules,
  getGuideStructureRules,
  isValidArticleFormat,
  isValidWhitepaperFormat,
  isValidNewsletterFormat,
  isValidStoryFormat,
  isValidGuideFormat,
  type BlogFormatType,
  type ArticleFormatType,
  type WhitepaperFormatType,
  type NewsletterFormatType,
...
type TemplateSectionSummary = { title: string; blockTypes: string[]; hints: string[] };
...
export function buildAnglesSystemPrompt(contentType: 'blog' | 'article' | 'whitepaper' | 'newsletter' | 'story' | 'guide' = 'blog'): string {
  ...
  const angle1 = isWhitepaper
    ...
    : isGuide ? '1. ANALYTICAL  — systematic, evidence-based deep dive — maps the landscape with data, frameworks, and structured analysis'
    : isStory ? '1. ANALYTICAL  — grounds the narrative in real data, research, or evidence — the story SHOWS the insight through concrete scenes'
    : isNewsletter ? '1. ANALYTICAL  — curates the best data, research, and evidence into actionable insights for the reader'
    : isArticle ? '1. ANALYTICAL  — investigative, examines evidence, data, and root causes with journalistic rigour'
  ...
  const angle2 = isWhitepaper
    ...
    : isGuide ? '2. CONTRARIAN  — challenges conventional approaches and presents a better path — "here\'s what most guides get wrong and how to actually do it"'
    : isStory ? '2. CONTRARIAN  — tells the story nobody expects — challenges assumptions through character choices, surprising turns, and unconventional perspectives'
    : isNewsletter ? '2. CONTRARIAN  — challenges what everyone assumes, surfaces overlooked perspectives and surprising counter-evidence'
    : isArticle ? '2. CONTRARIAN  — challenges the dominant narrative with original reporting and counter-evidence'
  ...
  const angle3 = isWhitepaper
    ...
    : isGuide ? '3. STRATEGIC   — frames the topic as a capability to build — connects knowledge to outcomes, includes implementation paths and real-world application'
    : isStory ? '3. STRATEGIC   — frames the narrative as a transformation journey — a protagonist faces a strategic challenge and emerges with actionable wisdom'
    : isNewsletter ? '3. STRATEGIC   — frames the topic as a practical playbook — what to do, when, and why it matters for the reader\'s goals'
    : isArticle ? '3. STRATEGIC   — frames the story through a business-impact lens with expert sourcing and analysis'
...
export function buildGenerationSystemPrompt(
  ...
  const identity = isWhitepaper
    ? `You are a senior research analyst, strategy consultant, and technical writer specializing in executive-level business publications. Today is ${currentYear}.\n\nYour task: generate a complete, publication-ready whitepaper that reads like it was produced by a top-tier consultancy — rigorous methodology, data-driven insights, executive summary, and actionable strategic recommendations.`
    : isGuide
    ? `You are a subject matter expert, pillar content architect, and technical educator. Today is ${currentYear}.\n\nYour task: generate a complete, publication-ready guide that reads like the definitive resource on its topic — comprehensive coverage, progressive structure, practical depth, and evergreen authority.`
    : isStory
    ? `You are a narrative storyteller, creative writer, and brand voice specialist. Today is ${currentYear}.\n\nYour task: generate a complete, publication-ready story that reads like it was written by a skilled author — vivid scenes, authentic dialogue, emotional depth, and a clear narrative arc.`
    : isNewsletter
    ? `You are a newsletter editor, audience engagement specialist, and industry curator. Today is ${currentYear}.\n\nYour task: generate a complete, publication-ready newsletter that feels like it was written by a trusted industry insider — conversational tone, curated insights, and actionable takeaways.`
    : isArticle
    ? `You are a senior journalist, investigative reporter, and industry analyst. Today is ${currentYear}.\n\nYour task: generate a complete, publication-ready article with journalistic depth — balanced perspectives, cited sources, and original analysis.`
  ...
  const toneRule = isWhitepaper
    ? `4. **Executive tone**: Formal, authoritative, data-driven. Present findings with institutional credibility. Never use: "game-changing", "revolutionary", "leverage", "synergy", "cutting-edge".`
    : isGuide
    ? `4. **Authoritative educator tone**: Clear, thorough, and practical. Write like the best teacher you've ever had. Never use: "game-changing", "revolutionary", "leverage", "synergy".`
    : isStory
    ? `4. **Narrative tone**: Authentic, human, experiential. Show, don't tell. Never use: "game-changing", "revolutionary", "leverage", "synergy".`
    : isNewsletter
    ? `4. **Conversational tone**: Direct, warm, audience-first. Write like a trusted colleague. Never use: "game-changing", "revolutionary", "leverage", "synergy".`
    : isArticle
    ? `4. **Journalistic tone**: Balanced, evidence-based, multi-perspective. Never promotional. Never use: "game-changing", "revolutionary", "leverage", "synergy".`
...
- **Methodology transparency**: State how findings were derived — survey data, market analysis, case study review, or expert synthesis.
- **Data density**: Every major claim must be supported by a data point, named source, or quantified example.
...
- **Progressive structure**: Build from foundational concepts to advanced techniques.
- **Concrete examples in every section**: Every concept must include at least one concrete example, code snippet, real-world scenario, or case study.
- **Actionable takeaways**: Each major section must end with specific, implementable advice.
- **Cross-referencing**: Refer back to earlier sections to create a cohesive learning path.
- **Definitions and terminology**: Define technical terms when first introduced.
- **Common mistakes**: Include "what NOT to do" guidance.
- **Practical depth over breadth**: Go deep enough that a reader could act on the advice immediately.
` : isNewsletter ? `
## NEWSLETTER-SPECIFIC STANDARDS
- **Audience-first**: Every section should answer "why should the reader care about this RIGHT NOW?"
- **Scannability**: Use bold key phrases, short paragraphs (2–4 sentences), and clear section headings.
- **Original commentary**: Don't just curate — add your perspective and tell the reader what it means for them.
- **Actionability**: Each section should leave the reader with something to DO, THINK, or SHARE.
- **Conversational warmth**: Write in second person. Use contractions. Feel like a trusted advisor.
- **Mobile-first**: Short paragraphs, clear hierarchy, no walls of text.
` : isStory ? `
## STORY-SPECIFIC CRAFT STANDARDS
- **Show, don't tell**: Use sensory details, dialogue, and action to convey meaning.
- **Emotional arc**: Start with tension or curiosity, build through complications, resolve with insight.
- **Concrete, specific details**: Ground the narrative in specific names, places, numbers, textures.
- **Natural dialogue**: When using dialogue, make it sound like real humans talking.
- **Strategic subtlety**: The brand message must emerge naturally from the narrative.
- **Pacing**: Vary sentence length. Short punchy sentences for impact.
- **Character through action**: Develop characters through what they DO and CHOOSE.
` : isArticle ? `
## ARTICLE-SPECIFIC JOURNALISTIC STANDARDS
- **Multiple perspectives**: Each major section should present at least one alternative viewpoint.
- **Attribution**: When making claims, attribute them — "According to X", "Research from Y shows".
- **Evidence density**: Prefer concrete data points, named studies, and expert quotes.
- **Balanced conclusion**: The summary should synthesize perspectives, not push a single narrative.
- **Original analysis**: Connect dots, identify patterns, and draw non-obvious conclusions.
` : '';
...
This is the #1 constraint. The finished article MUST be between ${Math.round(tw * 0.9)} and ${Math.round(tw * 1.1)} words.${tw >= 1600 ? `\n\nFor a ${tw}-word article, each of the ${minSections}–${maxSections} H2 sections must average ${wordsPerSection} words.` : ''}

## NON-NEGOTIABLE RULES

1. **No hallucination**: Never invent statistics, company names, or study results.
2. **No filler**: Every sentence must earn its place.
3. **Narrative construction**: Build an argument progressively. Each section must logically lead to the next.
${toneRule}
5. **Write for NOW**: The current year is ${currentYear}. All content must reflect the current state of the market.
6. **No year-anchored titles**: Never put a past year in the title or H2 headings.
${structureRule}
${articleExtra}
${depthRules}
...
- **Key Insights must be quotable**: Each bullet must be a self-contained, authoritative statement an AI could cite directly.
- **References must be real and authoritative**: Use well-known publications, research firms, or reputable industry platforms. Minimum ${isWhitepaper ? 5 : (isArticle || isGuide) ? 5 : 3} references.
- **Use <blockquote> for key data points or expert-level claims** — at least ${isWhitepaper ? '3–5' : isGuide ? '2–4' : isNewsletter ? '1–2' : isArticle ? '2–3' : '1–2'} blockquotes in the article.
- **Summary must be a standalone synthesis**: A reader (or AI) who only reads the Summary should understand the article's core argument.
...
export function buildGenerationUserPrompt(input: BlogGenerationInput): string {
  ...
  if (input.unifiedPromptContext) {
    ...
    if (input.performanceLearningsPrompt) lines.push(`\n${input.performanceLearningsPrompt}`);
    if (input.keywordContextPrompt) lines.push(`\n${input.keywordContextPrompt}`);
    if (input.trendContextPrompt) lines.push(`\n${input.trendContextPrompt}`);
    if (input.freshnessDirective) lines.push(`\n${input.freshnessDirective}`);
